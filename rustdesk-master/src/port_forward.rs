        ...
        tokio::select! {
            res = timeout(READ_TIMEOUT, stream.next()) => match res {
                ...
                Ok(Some(Err(err))) => {
                    ...
                    // Remove the return statement to continue the loop
...
async fn run_forward(mut forward: Framed<TcpStream, BytesCodec>, mut stream: Stream) -> ResultType<()> {
    let (mut forward_tx, mut forward_rx) = forward.split();
    let (mut stream_tx, mut stream_rx) = stream.split();
    let f2s = forward_tx.send_all(&mut stream_rx);
    let s2f = stream_tx.send_all(&mut forward_rx);
    match tokio::try_join!(f2s, s2f) {
        Ok(_) => Ok(()),
        Err(err) => {
            log::error!("Error running forward: {:?}", err);
            bail!("Forward error: {:?}", err);
        }
    }
}
